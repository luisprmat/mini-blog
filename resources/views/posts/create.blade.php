<x-app-layout :title="__('New post')" :meta-description="__('Create new post')">
    <h1 class="my-4 text-center font-serif text-3xl text-sky-600 dark:text-sky-500">{{ __('Create new post') }}</h1>

    <form class="mx-auto max-w-xl rounded bg-white px-8 py-4 shadow dark:bg-slate-800" action="{{ route('posts.store') }}"
        method="POST">
        @csrf
